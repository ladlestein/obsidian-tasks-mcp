...
TaskRegex.indentationRegex = /^([\s\t>]*)/;
// Matches - * and + list markers, or numbered list markers, for example 1. and 1)
TaskRegex.listMarkerRegex = /([-*+]|[0-9]+[.)])/;
// Matches a checkbox and saves the status character inside
TaskRegex.checkboxRegex = /\[(.)\]/u;
// Matches the rest of the task after the checkbox.
TaskRegex.afterCheckboxRegex = / *(.*)/u;
// Main regex for parsing a line. It matches the following:
// - Indentation
// - List marker
// - Status character
// - Rest of task after checkbox markdown
TaskRegex.taskRegex = new RegExp(TaskRegex.indentationRegex.source +
    TaskRegex.listMarkerRegex.source +
    ' +' +
    TaskRegex.checkboxRegex.source +
    TaskRegex.afterCheckboxRegex.source, 'u');
// Matches hashtags in task descriptions
TaskRegex.hashTags = /(^|\s)#[^ !@#$%^&*(),.?":{}|<>]+/g;
// Date related regular expressions - matches emoji followed by date
TaskRegex.dueDateRegex = /[📅🗓️]\s?(\d{4}-\d{2}-\d{2})/;
TaskRegex.scheduledDateRegex = /⏳\s?(\d{4}-\d{2}-\d{2})/;
TaskRegex.startDateRegex = /🛫\s?(\d{4}-\d{2}-\d{2})/;
TaskRegex.createdDateRegex = /➕\s?(\d{4}-\d{2}-\d{2})/;
// Priority emoji - order is important! Longest pattern first
TaskRegex.priorityRegex = /(⏫⏫|⏫|🔼|🔽|⏬)/g;
// Recurrence
TaskRegex.recurrenceRegex = /🔁\s?(.*?)(?=(\s|$))/;
/**
 ...
 */
export function parseTasks(text, filePath = '') {
 ...
 */
export function parseTaskLine(line, filePath = '', lineNumber = 0) {
