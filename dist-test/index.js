...
import { parseTasks, queryTasks as filterTasks } from './TaskParser.js';
export async function findAllMarkdownFiles(startPath) {
    const pattern = path.join(startPath, '**/*.md');
    return glob(pattern);
}
export async function extractTasksFromFile(filePath) {
    try {
...
export async function findAllTasks(directoryPath) {
...
export function queryTasks(tasks, queryText) {
    try {
...
export function serializeTasksToJson(tasks) {
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: "list_all_tasks",
                description: "Extract all tasks from markdown files in a directory. " +
                ...
                inputSchema: zodToJsonSchema(ListAllTasksArgsSchema),
            },
            {
                name: "query_tasks",
                description: "Search for tasks based on Obsidian Tasks query syntax. " +
                    ...
                    "The path parameter is optional; if not specified, it defaults to the vault root directory. " +
                    "The path must be relative to the vault directory and cannot contain directory traversal components (..).",
                inputSchema: zodToJsonSchema(QueryTasksArgsSchema),
            }
        ],
    };
});
// Exported handlers for testing
export async function handleListAllTasksRequest(args) {
    try {
...
export async function handleQueryTasksRequest(args) {
    try {
